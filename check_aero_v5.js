...
async function check() {
    ...
    const factory = '0x420DD3807E0e1055ADed95A3c9ef28210904A025'.toLowerCase();
    
    // Try a different factory (Slipstream)
    const slipstreamFactory = '0x5e787D239BB2279f0B7D22381Ed95E18ab3f210c'.toLowerCase();
    const routes = [{ from: t0, to: t1, stable: false, factory: slipstreamFactory }];
    
    try {
        const amounts = await router.getAmountsOut(amountIn, routes);
        console.log('✅ Aerodrome V5 Success! Amount out:', ethers.formatUnits(amounts[1], 6));
    } catch (e) {
        console.log('❌ Aerodrome V5 Failed:', e.message);
    }
}
