...
import fetch from 'node-fetch'; // Still needed for telegram within the module
...
// Pyth connection for price data (if needed by protocols for more complex health checks)
    ...
    try {
      ...
      for (const { pubkey, programId } of obligations) {
        ...
        if (!activeObligationSubscriptions.has(pkStr)) {
          ...
          // console.log(`Subscribed to ${protocol.name} obligation: ${pkStr}`);
        }
      }
    } catch (e: any) {
...
// Handler for obligation account changes (main health check logic)
...
process.on('SIGINT', async () => {
  ...
  for (const [pkStr, subId] of activeObligationSubscriptions.entries()) {
    ...
    // console.log(`Removed account change listener for ${pkStr}`);
  }
