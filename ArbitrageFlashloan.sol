...
contract ArbitrageFlashloan {
    ...
    function initiateFlashloan(TradeParams calldata params) external onlyOwner {
        uint256 totalRepay = params.amountBorrow; // Aave V3 premium is handled in executeOperation
    ...
    function executeOperation(
        ...
        if (!successA) {
            // If Aerodrome fails, try Uniswap (the bot handles the order, but we check both)
