...
class ArbitrageBot {
  ...
  parseAmount(amount, decimals) { 
    try {
      ...
      console.error(chalk.red(`❌ Error parsing amount ${amount} with decimals ${decimals}:`), error.message);
  ...
  async getUniswapQuote(tokenIn, tokenOut, amountIn, fee) {
    try {
      ...
      console.warn(chalk.yellow(`⚠️  Uniswap quote failed for ${tokenIn.name || tokenIn.symbol}/${tokenOut.name || tokenOut.symbol}: ${e.message}`));
  ...
  async getAerodromeQuote(tokenIn, tokenOut, amountIn) {
    try {
      ...
      // FIX: Better error logging with token names
      console.warn(chalk.yellow(`⚠️  Aerodrome quote failed for ${tokenIn.name || tokenIn.symbol}/${tokenOut.name || tokenOut.symbol}: ${e.message}`));
      return { success: false }; 
    }
  }
  ...
  async calculateNetProfit(tokenIn, tokenOut, amountIn, fee) {
    ...
    if (netProfit > 0 && netProfit < 1000000) { // Sanity check: profit shouldn't be > $1M from $1k
      ...
      console.log(chalk.red(`  ❗ Unrealistic profit detected: $${netProfit.toFixed(2)} - likely pricing error`));
      return -1;
    }
  ...
  async executeArbitrage(pair, optimalSize, netProfit) {
    ...
    if (!this.wallet || !this.arbitrageContract) {
      console.log(chalk.yellow(`⚠️  READ-ONLY MODE: Opportunity found for ${pair.name} ($${netProfit.toFixed(2)}), but no wallet or contract configured.`));
      return;
    }

    // Safety check: Don't execute if profit seems unrealistic
    if (netProfit > 1000) {
      console.log(chalk.red(`⚠️  Suspicious profit detected: $${netProfit.toFixed(2)} from $${optimalSize.toFixed(2)} - skipping execution`));
      return;
    }

    console.log(chalk.yellow.bold(`🚀 EXECUTING MEV-PROTECTED ARBITRAGE: ${pair.name}`));
    ...
    try {
      ...
      if (error.code === 'INSUFFICIENT_FUNDS') {
        ...
        console.error(chalk.red('  ❗ Chain ID mismatch. Check your RPC URL and network configuration.'));
      }
    }
  }
  ...
  async checkPair(pair) {
    ...
    if (pair.enabled === false) {
      console.log(chalk.gray(`  Skipping disabled pair: ${pair.name}`));
      return;
    }
    ...
    try {
      ...
      if (maxNetProfit > config.settings.executionThreshold) {
        ...
        console.log(chalk.green(`  Pair: ${pair.name}`));
      ...
      console.error(chalk.red(`❌ Error scanning pair ${pair.name}:`), error.message);
    }
  }
  ...
  async monitor() {
    ...
    console.log(chalk.blue(`Scanning ${config.pairs.filter(p => p.enabled !== false).length} enabled pairs every ${config.settings.scanInterval / 1000}s`));
    ...
    try {
      ...
      if (this.wallet) {
        ...
        if (ethers.formatEther(balance) < 0.01) {
          console.warn(chalk.yellow('⚠️  Low balance - ensure you have enough ETH for gas fees'));
        }
      }
    } catch (error) {
