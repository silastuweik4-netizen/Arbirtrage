// ArbitrageBot.js - FULL UPDATED VERSION (Jan 15, 2026)
// Includes Aerodrome quote logging, profit calc logging, all previous fixes
...
class ArbitrageBot {
  ...
  async calculateNetProfit(tokenIn, tokenOut, amountIn, fee) {
    ...
    if (uniSuccess && aeroSuccess) {
      amountOutA = uniResult.value.amountOut > aeroResult.value.amountOut 
        ? uniResult.value.amountOut 
        : aeroResult.value.amountOut;
    ...
    if (isUniForA || (!aeroSuccess && uniSuccess)) {
      const reverseResult = aeroSuccess 
      ...
      const reverseResult = uniSuccess 
        ? await this.getUniswapQuote(tokenOut, tokenIn, this.formatAmount(amountOutA, tokenOut.decimals), fee)
        : await this.getAerodromeQuote(tokenOut, tokenIn, this.formatAmount(amountOutA, tokenOut.decimals));
      amountOutB = reverseResult.amountOut || 0n;
    }
    ...
    // NEW: Log actual profit calculation
    console.log(chalk.gray(
      `[${new Date().toISOString()}] Profit calc for ${amountIn} ${tokenIn.symbol} → ${tokenOut.symbol}: ` +
  ...
  async estimateGasCost() {
    try {
      ...
      return gasCostEth * (config.settings.ethPriceUsd || 3000);
    } catch {
      return 0.30;
    }
  }
  ...
  async executeArbitrage(pair, optimalSize, netProfit) {
    ...
    try {
      ...
      const minRepaymentAmount = amountBorrow + (amountBorrow * 5n) / 10000n;
      ...
      if (!uniQuote.success && !aeroQuote.success) {
        console.log(chalk.red(`❌ No usable quotes for ${pair.token0}/${pair.token1}. Skipping.`));
        return;
      }
      ...
      let expectedIntermediateAmount = 0n;
      ...
      if (uniQuote.success && aeroQuote.success) {
        if (uniQuote.amountOut < aeroQuote.amountOut) {
          ...
          const reverseAero = await this.getAerodromeQuote(token1, token0, this.formatAmount(expectedIntermediateAmount, token1.decimals));
          const minOutB = reverseAero.success ? (reverseAero.amountOut * BigInt(10000 - SLIPPAGE_TOLERANCE_BPS)) / 10000n : 0n;
          ...
          const minOutA = (expectedIntermediateAmount * BigInt(10000 - SLIPPAGE_TOLERANCE_BPS)) / 10000n;
          const reverseUni = await this.getUniswapQuote(token1, token0, this.formatAmount(expectedIntermediateAmount, token1.decimals), pair.fee);
          const minOutB = reverseUni.success ? (reverseUni.amountOut * BigInt(10000 - SLIPPAGE_TOLERANCE_BPS)) / 10000n : 0n;
        ...
        console.log(chalk.yellow(`Only Uniswap quote succeeded for ${pair.token0}/${pair.token1}. Checking reverse...`));
        ...
        if (reverseUniQuote.success) {
          ...
          console.log(chalk.red(`No reverse Uniswap quote. Skipping.`));
          return;
        }
      } else {
        console.log(chalk.yellow(`Only Aerodrome quote succeeded for ${pair.token0}/${pair.token1}. Attempting round-trip on Aerodrome.`));
        ...
        if (reverseAeroQuote.success) {
          ...
          console.log(chalk.red(`No reverse Aerodrome quote. Skipping.`));
          return;
        }
      }
      ...
      if (receipt.status === 1) {
        ...
        if (config.settings.oneShotMode && config.settings.stopAfterFailure) {
          console.log(chalk.yellow('\n⏹️ Stopping after failed execution'));
          this.stop();
        }
      }
  ...
  async monitor() {
    ...
    while (this.isRunning) {
      for (const pair of config.pairs) {
        console.log(chalk.gray(`[${new Date().toISOString()}] Checking pair: ${pair.name}`));
  ...
  async start() {
    try {
      ...
      if (this.wallet) {
        ...
        if (parseFloat(ethBalance) < config.settings.minBalanceToOperate) {
          console.log(chalk.red(`⚠️ WARNING: Balance below minimum (${config.settings.minBalanceToOperate} ETH)`));
        }
      }
    } catch (error) {
