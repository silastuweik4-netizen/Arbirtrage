...
class ArbitrageBot {
  ...
  async executeArbitrage(pair, optimalSize, netProfit) {
    ...
    console.log(colors.yellow + bold + '🚀 EXECUTING MEV-PROTECTED ARBITRAGE: ' + pair.name + reset);
    ...
    try {
      ...
      if (!uniQuote.success || !aeroQuote.success) {
        console.error(colors.red + '❌ Failed to get quotes for execution' + colors.reset);
        return;
      }
  ...
  async checkPair(pair) {
    ...
    try {
      ...
      if (maxNetProfit > config.settings.executionThreshold) {
        console.log(colors.green + bold + '🎯 ARBITRAGE OPPORTUNITY FOUND!' + reset);
  ...
  async monitor() {
    console.log(colors.blue + bold + '\n🤖 Base Arbitrage Bot: MEV PROTECTION ACTIVE' + reset);
    ...
    while (this.isRunning) {
      ...
      console.log(colors.magenta + bold + '🔁 Scan Cycle #' + cycle + reset);
      console.log(colors.magenta + bold + '⏰ ' + new Date().toLocaleTimeString() + reset);
