...
class ArbitrageBot {
  constructor() {
    ...
    try {
      // ----- Lava-Net header injection for ethers v6 -----
      const customFetch = (url, init) =>
        fetch(url, { ...init, headers: { ...init?.headers, Authorization: 'Bearer 74c33b48f194b4900d1b1d4b108fd2ae' } });

      this.provider = new ethers.JsonRpcProvider(scanRpcUrl, chainId, {
        staticNetwork: true,
  ...
  async getUniswapQuote(tokenIn, tokenOut, amountIn, fee) {
    if (!tokenIn || !tokenOut) { console.log('[UNI] Missing token config', tokenIn, tokenOut); return { success: false }; }
    try {
      const params = { tokenIn: tokenIn.address, tokenOut: tokenOut.address, amountIn: this.parseAmount(amountIn, tokenIn.decimals), fee, sqrtPriceLimitX96: 0 };
    ...
    } catch (e) { console.log(`[UNI] ${tokenIn.symbol}→${tokenOut.symbol}  ERROR:`, e.code, e.message); return { success: false }; }
  }

  async getAerodromeQuote(tokenIn, tokenOut, amountIn) {
    if (!tokenIn || !tokenOut) { console.log('[AERO] Missing token config', tokenIn, tokenOut); return { success: false }; }
    try {
      ...
      return { amountOut: parseFloat(out), success: true };
    } catch (e) { console.log(`[AERO] ${tokenIn.symbol}→${tokenOut.symbol}  ERROR:`, e.code, e.message); return { success: false }; }
  }

  async calculateNetProfit(tokenIn, tokenOut, amountIn, fee) {
    const [uni, aero] = await Promise.all([this.getUniswapQuote(tokenIn, tokenOut, amountIn, fee), this.getAerodromeQuote(tokenIn, tokenOut, amountIn)]);
  ...
  async findOptimalSize(pair) {
    ...
    if (!token0 || !token1) { console.log('Missing token config for pair', pair); return { bestSize: 0, maxNetProfit: -1 }; }
  ...
  encodeAerodromeSwap(tokenIn, tokenOut, amountIn, amountOutMin) {
    const iface = new ethers.Interface(['function swapExactTokensForTokens(uint256 amountIn,uint256 amountOutMin,(address from,address to,bool stable,address factory)[] routes,address to,uint256 deadline) external returns (uint256[])']);
    const routes = [{ from: tokenIn.address, to: tokenOut.address, stable: false, factory: config.contracts.aerodromeFactory }];
  ...
  encodeUniswapSwap(tokenIn, tokenOut, amountIn, amountOutMin, fee) {
    ...
    const deadline = Math.floor(Date.now() / 1000) + 60 * 20;
    const params = { tokenIn: tokenIn.address, tokenOut: tokenOut.address, fee, recipient: config.contracts.arbitrageContract, deadline, amountIn, amountOutMinimum: amountOutMin, sqrtPriceLimitX96: 0 };
  ...
  async executeArbitrage(pair, optimalSize, netProfit) {
    ...
    try {
      ...
      const tradeParams = { tokenBorrow: token0.address, amountBorrow, tokenIn: token0.address, tokenOut: token1.address, minAmountOut, swapDataA, swapDataB };
    ...
    } catch (error) { console.error(chalk.red('❌ Execution Error:'), error.message); }
  }

  async checkPair(pair) { const { bestSize, maxNetProfit } = await this.findOptimalSize(pair); if (maxNetProfit > config.settings.executionThreshold) await this.executeArbitrage(pair, bestSize, maxNetProfit); }

  async monitor() {
    console.log(chalk.blue.bold('\n🤖 Base Arbitrage Bot: MEV PROTECTION ACTIVE'));
    this.isRunning = true;
    while (this.isRunning) { await Promise.all(config.pairs.map(p => this.checkPair(p))); await new Promise(r => setTimeout(r, config.settings.updateInterval)); }
  }

  async start() {
    try { const network = await this.provider.getNetwork(); console.log(chalk.green(`✅ Connected to Base Network (Chain ID: ${network.chainId})`)); } catch (error) { console.error(chalk.red('❌ Connection Failed:'), error.message); process.exit(1); }
    await this.monitor();
  }
}
