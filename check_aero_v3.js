...
async function check() {
    ...
    const routes = [{ from: t0, to: t1, stable: false, factory: factory }];
    
    try {
        const amounts = await router.getAmountsOut(amountIn, routes);
        console.log('✅ Aerodrome V3 Success! Amount out:', ethers.formatUnits(amounts[1], 6));
    } catch (e) {
        console.log('❌ Aerodrome V3 Failed:', e.message);
    }
}
