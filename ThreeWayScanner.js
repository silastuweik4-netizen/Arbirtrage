...
// ---------- hard-map Aerodrome Slipstream pools (same 20 we scanned) ----------
const AERO_POOLS = {
...
class ThreeWayScanner {
  ...
  async getQuote(dex, tokenIn, tokenOut, fee, amountIn) {
    try {
      ...
      if (dex === 'AERO') {
        ...
        if (!pool) { console.warn('Aero pool not mapped', tokenIn, tokenOut, fee); return 0n; }
        ...
        return BigInt(Math.floor(price * Number(amountIn)));
      }
      ...
      console.error(`Quote fail: ${dex} ${tokenIn}->${tokenOut} ${fee}`, e.message);
      return 0n;
    }
  }
  ...
  async scanForArbitrageOpportunities() {
    ...
    for (const pairData of this.profitCalc.PAIR_DATA) {
      // Get best quotes from each DEX (parallel)
