...
async function scanAllPairs() {
  console.log('🚀 Starting Comprehensive Scan of 55+ Token Pairs on Base Network\n');
  ...
  for (const pair of config.pairs) {
    ...
    // Determine a reasonable test size for the token
    let testSize = 100.0;
    if (t0.symbol === 'WETH' || t0.symbol === 'cbETH' || t0.symbol === 'wstETH' || t0.symbol === 'rETH' || t0.symbol === 'ezETH' || t0.symbol === 'weETH' || t0.symbol === 'LsETH') {
      testSize = 1.0;
    } else if (t0.symbol === 'cbBTC' || t0.symbol === 'WBTC' || t0.symbol === 'tBTC') {
      testSize = 0.05;
    } else if (t0.symbol === 'YFI') {
      testSize = 0.01;
    }
    ...
    try {
      ...
      let profitInUSD = 0;
      if (t1.symbol === 'USDC' || t1.symbol === 'USDbC' || t1.symbol === 'DAI') {
        profitInUSD = grossProfit;
      } else if (t1.symbol === 'WETH' || t1.symbol === 'cbETH' || t1.symbol === 'wstETH') {
        profitInUSD = grossProfit * 3000; // Approx ETH price
      } else if (t1.symbol === 'WBTC' || t1.symbol === 'cbBTC') {
        profitInUSD = grossProfit * 90000; // Approx BTC price
      }

      const flashloanFee = testSize * buyPrice * 0.0005;
      const netProfitUSD = profitInUSD - (flashloanFee * (t1.symbol === 'WETH' ? 3000 : (t1.symbol === 'WBTC' ? 90000 : 1))) - 0.20;
