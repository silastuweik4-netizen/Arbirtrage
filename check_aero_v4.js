...
async function check() {
    ...
    // Try stable: true
    const routes = [{ from: t0, to: t1, stable: true, factory: factory }];
    
    try {
        const amounts = await router.getAmountsOut(amountIn, routes);
        console.log('✅ Aerodrome V4 Success! Amount out:', ethers.formatUnits(amounts[1], 6));
    } catch (e) {
        console.log('❌ Aerodrome V4 Failed:', e.message);
    }
}
