...
async function initializeServices() {
  try {
    ...
    // Ensure the scanner module sees the same RPC URL (in case it reads process.env at import)
    ...
    console.error('❌ Service initialization failed:', error && (error.message || String(error)));
    scannerStatus.error = error && (error.message || String(error));
  }
}
...
async function performScan() {
  ...
  try {
    ...
    scannerStatus.error = error && (error.message || String(error));
    console.error('❌ Scan error:', error && (error.message || String(error)));
  } finally {
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      '/api/diagnostics': 'GET - Diagnostics (RPC, init status)'
    }
  });
});
...
// Diagnostics - quick status for debugging on phone
...
app.listen(PORT, async () => {
  ...
  // If initialization failed, disable auto-scanning and surface the error
  if (scannerStatus.error || !scanner) {
    console.error('❌ Scanner failed to initialize — auto-scanning disabled. Fix initialization and restart.');
    return;
  }
