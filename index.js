...
const provider = new ethers.JsonRpcProvider(
  process.env.ALCHEMY_URL || `https://base-mainnet.g.alchemy.com/v2/${process.env.ALCHEMY_KEY}`
);
...
async function cronScan() {
  try {
    ...
    if (top.length) console.table(top);
    else console.log(`${new Date().toISOString()} – no diff-fee arb > 0 bp`);
  } catch (e) {
    console.error('Cron error:', e);
  }
}
cronScan();                                    // first run immediately
setInterval(cronScan, CRON_MINUTES * 60 * 1000);
