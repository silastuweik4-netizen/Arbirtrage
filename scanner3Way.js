// threeWayScanner.js
// Full updated scanner with normalized addresses, V3/Aerodrome ABIs,
...
class ThreeWayScanner {
  ...
  async _quickMultiDexTest(aeroAddr, uniAddr, pancakeAddr) {
    try {
      ...
      try {
        const outU = await this.uniswap.callStatic.quoteExactInputSingle(WETH, USDC, 500, amountIn, 0);
        console.log('UNI_QUOTE_OK', ethers.formatUnits(outU, 6));
      } catch (e) {
        console.log('UNI_QUOTE_ERR', e && (e.message || String(e)));
      }

      // PancakeSwap V3 QuoterV2 (single)
      try {
        const outP = await this.pancake.callStatic.quoteExactInputSingle(WETH, USDC, 500, amountIn, 0);
