// threeWayScanner.js
// 2026-01-17 Updated: Corrected Base Mainnet Quoter Addresses, Implemented scan function, and added Checksum normalization.
...
const QUOTER_ADDRESSES_RAW = {
  ...
  PANCAKESWAP: "0x7179D19E5244E11d886915e2e6b71B55b0998c0b" // PancakeSwap V3 QuoterV2
};
...
class ThreeWayScanner {
  constructor() {
    ...
    // Raw token config - capitalization will be corrected in buildPairDataFromConfig
  ...
  // Utility to normalize addresses (checksum)
  _normalizeAddress(addr) {
    try {
      return ethers.getAddress(addr);
    } catch (err) {
      console.error('Invalid address, cannot normalize:', addr);
      throw err;
    }
  }

  async initialize(defaultAmount = '1') {
    // Normalize quoter addresses during initialization
    ...
    // Normalization also happens inside buildPairDataFromConfig now
  ...
  buildPairDataFromConfig(defaultAmount) {
    ...
    for (const [pair, cfg] of this.pairConfigRaw.entries()) {
      list.push({
        pair,
        // Use the normalization function here
        token0: this._normalizeAddress(cfg.token0),
        token1: this._normalizeAddress(cfg.token1),
  ...
  async getQuote(dex, tokenIn, tokenOut, fee, amountIn) {
    ...
    try {
      ...
      console.error(`getQuote error for ${dex}: ${e.message}`);
      return 0n;
    }
  }
  ...
  async scanForArbitrageOpportunities() {
    ...
    for (const pair of this.profitCalc.PAIR_DATA) {
      for (const fee of FEE_TIERS) {
        ...
        // Evaluate opportunities (implementation assumed to be in ThreeWayArbitrageCalculator)
        if (quoteAero > 0n && quoteUni > 0n && quotePancake > 0n) {
           // Pass data to your calculator which handles logging/alerts
