...
// ==================== STATIC POOL LIST ====================
// This is our curated list of high-value pools to monitor.
const STATIC_POOLS = [
  // VIRTUAL Pools
  ...
  // NEW POOL ADDED HERE
  { dex: 'uniswap_v2', pairAddress: '0x081eC5d6b68A28C9e3a73eC97E351161dBB5e8ef', token0: TOKENS.AERO, token1: TOKENS.WETH },
];
...
class ArbitrageDetector {
  ...
  async scan() {
    // Use only the STATIC_POOLS list
    const allPairs = STATIC_POOLS;

    console.log(`\n[${new Date().toISOString()}] Scanning ${allPairs.length} static pairs & ${TRIANGULAR_ROUTES.length} triangular routes...`);
...
async function main() {
  ...
  // No more discovery, just scan the static list
  await detector.scan();
  ...
  http.createServer((req, res) => {
    ...
    res.end('Arbitrage Bot running: Static Pool List Mode.\n');
