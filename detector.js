...
// Minimal ABI for V2-style pairs (Aerodrome pools)
const PAIR_ABI = [
...
// ==================== AERODROME RESERVES HELPER ====================
/**
 * Query reserves directly from an Aerodrome pool.
 * @param {string} poolAddress - Address of the Aerodrome pool (pair contract).
 * @param {ethers.providers.Provider} provider - Ethers provider instance.
 * @returns {Promise<{reserve0: BigNumber, reserve1: BigNumber, token0: string, token1: string}>}
 */
...
// ==================== RESERVE VALUATION ====================
/**
 * Convert Aerodrome reserves into USD value using TOKEN_PRICES_USD.
 * @param {BigNumber} reserve0 - Reserve amount of token0
 * @param {BigNumber} reserve1 - Reserve amount of token1
...
function reservesToUSD(reserve0, reserve1, token0, token1) {
  ...
  const price0 = TOKEN_PRICES_USD[tokenMeta0.name] || 1;
  const price1 = TOKEN_PRICES_USD[tokenMeta1.name] || 1;
...
// ==================== LOGGING UTILITY ====================
/**
 * Log liquidity checks in a consistent format.
 * @param {string} poolLabel - Human-readable pool label (e.g., "msETH/WETH Aerodrome")
 * @param {number} liquidityUSD - Total liquidity in USD
 * @param {number} threshold - Minimum liquidity threshold in USD
 */
...
class ArbitrageDetector {
  ...
  async _checkPool(pool) {
    try {
      ...
      // Liquidity validation for Aerodrome pools
      if (pool.dex === "aerodrome") {
        const { reserve0, reserve1, token0, token1 } = await getAerodromeReserves(pool.pairAddress, provider);
        const liquidityUSD = reservesToUSD(reserve0, reserve1, token0, token1);
        logLiquidityCheck(`${pool.token0.name}/${pool.token1.name} ${pool.dex}`, liquidityUSD, CONFIG.MIN_LIQUIDITY_USD);
        if (liquidityUSD < CONFIG.MIN_LIQUIDITY_USD) {
          console.log(`Skipping ${pool.token0.name}/${pool.token1.name} due to low USD liquidity`);
          return;
        }
      }
  ...
  async _checkTriangular(route) {
    try {
      ...
      for (const leg of route.legs) {
        // Default to Uniswap V3 for quoting legs (as per your original logic)
        const pool = { dex: "uniswap_v3", token0: leg.tokenIn, token1: leg.tokenOut, meta: { feeTiers: [3000] } };

        // If a leg is Aerodrome (you can adapt detection if needed), validate reserves
        if (pool.dex === "aerodrome" && pool.pairAddress) {
          const { reserve0, reserve1, token0, token1 } = await getAerodromeReserves(pool.pairAddress, provider);
          const liquidityUSD = reservesToUSD(reserve0, reserve1, token0, token1);
          logLiquidityCheck(`${token0}/${token1} ${pool.dex} (Triangular leg)`, liquidityUSD, CONFIG.MIN_LIQUIDITY_USD);
          if (liquidityUSD < CONFIG.MIN_LIQUIDITY_USD) {
            console.log(`Skipping triangular leg ${token0}/${token1} due to low USD liquidity`);
            return;
          }
        }
  ...
  _routerForVenue(venue) {
    if (venue === "uniswap_v3") return DEX_ADDRESSES.UNISWAP_V2_ROUTER; // SwapRouter for execution (placeholder per original)
