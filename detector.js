...
class ArbitrageDetector {
  ...
  // Get trading pair symbol by exchange (contract addresses are for reference only)
  getTradingSymbol(pair, exchange) {
    // Exchanges use different pair formats:
    // Binance & MEXC: BTCUSDT (no separator)
    // KuCoin: BTC-USDT (with dash separator)
    
    const baseQuote = `${pair.base}${pair.quote}`;
  ...
  detectArbitrage(prices) {
    ...
    this.monitoringPairs.forEach((pair) => {
      ...
      const binanceSymbol = this.getTradingSymbol(pair, 'binance');
      const mexcSymbol = this.getTradingSymbol(pair, 'mexc');
      const kucoinSymbol = this.getTradingSymbol(pair, 'kucoin');
...
export default detector; + parseFloat(binancePrice).toFixed(8) : '❌ NOT FOUND'}`
      );
      console.log(
        `    mexc     → ${mexcPrice ? '
...
// Export for server integration
export default detector; + parseFloat(mexcPrice).toFixed(8) : '❌ NOT FOUND'}`
      );
      console.log(
        `    kucoin   → ${kucoinPrice ? '
...
// Export for server integration
export default detector; + parseFloat(kucoinPrice).toFixed(8) : '❌ NOT FOUND'}`
      );
    });
