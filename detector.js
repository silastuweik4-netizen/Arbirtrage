...
http.createServer((req, res) => {
  ...
  res.end('Base Arbitrage Detector is running...');
}).listen(PORT, () => {
  console.log(`✅ Web Server active on port ${PORT}. Render health checks will pass.`);
});
...
const ORACLES = {
  ...
  DOT: '0xF301d2c18090C8330761a2F6564619E423b49984',
  AAVE: '0x321E5c589000a6e0B92B08f5d023fF70570b284C',
  CRV: '0x157835154378A6b82531d2794D8781B88B27F47e',
  LDO: '0x9953949f50689b0270a64936E5a60e0aF47E84C1'
};

// ==================== TOKEN LIST (36 Tokens) ====================
const TOKENS = {
  ...
  AERO: { address: '0x940181a94A35A4569E4529A3CDfB74e38FD98631', decimals: 18, oracle: 'WETH' }, // Approx AERO/USD via WETH/USD
  ...
  // Added more liquid/popular tokens on Base
  DOT: { address: '0x321e5c589000a6e0b92b08f5d023ff70570b284c', decimals: 10, oracle: 'DOT' }, 
  AAVE: { address: '0x321e5c589000a6e0b92b08f5d023ff70570b284c', decimals: 18, oracle: 'AAVE' },
  CRV: { address: '0x157835154378a6b82531d2794d8781b88b27f47e', decimals: 18, oracle: 'CRV' },
  LDO: { address: '0x9953949f50689b0270a64936e5a60e0af47e84c1', decimals: 18, oracle: 'LDO' },
  RIVER: { address: '0xe2598363a1523c5e12fC195155A5D7C1d6168F55', decimals: 18, oracle: 'WETH' },
  MORPHO: { address: '0x3508b79A348a044709405F7B81C990176D5d8a9e', decimals: 18, oracle: 'WETH' },
  CAKE: { address: '0x104b2b3a164e81561f5f3df0d3c01c0c66d2105e', decimals: 18, oracle: 'WETH' },
  ZRO: { address: '0x446D709e3e70E83d0F691A68d201E78832a26c48', decimals: 18, oracle: 'WETH' },
  W: { address: '0x40d16ae08B01a2E35160e1d0092c249a2a904000', decimals: 18, oracle: 'WETH' },
  PENDLE: { address: '0x498c8c56c2c77a3d3c636f1c4e235e197d130ed8', decimals: 18, oracle: 'WETH' }
};
...
class ArbDetector {
  ...
  async scan() {
    console.log(`\n[${new Date().toLocaleTimeString()}] Scanning ${Object.keys(TOKENS).length} Tokens...`);
          ...
          for (const d2 of dexes) {
            ...
            if (diff >= CONFIG.PRICE_DIFF_THRESHOLD) {
              ...
              if (realizableProfitPct >= CONFIG.MIN_REALIZABLE_PROFIT_PCT) {
                const msg = `💰 EXECUTE: ${tKeys[i]}/${tKeys[j]} | Realizable: ${realizableProfitPct.toFixed(2)}% | Buy ${d1} -> Sell ${d2}`;
                console.log(msg);
