 ...
 * Add or override taker fees via env variables if needed.
 */
const exchanges = {
  binance: {
    name: 'Binance',
...
class ArbitrageDetector {
  ...
  async fetchDeepcoin() {
    try {
      ...
      else if (raw && raw.result && Array.isArray(raw.result)) tickers = raw.result;
      else if (raw && typeof raw === 'object') {
        // keyed object: { "BTCUSDT": { last: "123" }, ... }
        tickers = Object.entries(raw).map(([symbol, obj]) => {
          return { symbol, last: obj?.last ?? obj?.price ?? obj?.close ?? obj?.lastPrice ?? null };
        });
      } else {
        // Fallback: try nested data fields
        const candidate = raw?.data?.tickers ?? raw?.data?.result ?? raw?.tickers ?? raw?.result;
        if (Array.isArray(candidate)) tickers = candidate;
        else throw new Error('Unexpected Deepcoin response shape');
      }

      const prices = {};
      tickers.forEach(t => {
        const symbolRaw = t.symbol || t.pair || t.market || t.marketName || t.symbolName;
        const priceRaw = t.last ?? t.price ?? t.close ?? t.lastPrice ?? t.last_trade_price ?? t.value;
    ...
    for (const pair of this.monitoringPairs) {
      ...
      for (let i = 0; i < exList.length; i++) {
        for (let j = i + 1; j < exList.length; j++) {
          ...
          const transferCostUsd = parseFloat(process.env.TRANSFER_COST_USD) || 0;
