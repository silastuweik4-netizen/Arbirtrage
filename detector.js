...
class PriceFetcher {
  ...
  async getSwapUniswapV3(tokenIn, tokenOut, amountIn, feeTiers = [100, 500, 3000, 10000]) {
    try {
      for (const fee of feeTiers) {
        try {
          const amountOut = await this.quoterV3.quoteExactInputSingle(tokenIn, tokenOut, fee, amountIn);
          return amountOut;
        } catch { continue; }
      }
      return null;
  ...
  async getPrice(token0, token1, dexType, tradeSize = CONFIG.TRADE_SIZE) {
    ...
    if (dexType === 'uniswap_v3') {
      return await this.getSwapUniswapV3(token0.address, token1.address, amountIn);
    ...
    for (const pair of pairs) {
      try {
        ...
        if (Math.abs(diff) >= CONFIG.PRICE_DIFFERENCE_THRESHOLD) {
          ...
          console.log(`    ℹ Spread: ${diff.toFixed(4)}% (below ${CONFIG.PR
