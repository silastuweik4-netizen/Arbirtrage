...
const DEX_ADDRESSES = {
  ...
  SQUADSWAP_QUOTER: '0x84ff9b09B56c3858c38Cc7Aa678FabB73B9f3898',
  ...
  SQUADSWAP_V2_ROUTER: '0xe58b1e7E398e8cC97a276C55Fb7aB5e46eE144f1',
};
...
function generatePairs() {
  ...
  for (const tName of majorTokens) {
    for (const sName of stablecoins) {
      pairs.push({ t0: TOKENS[tName], t1: TOKENS[sName], dexes: ['uniswap_v3', 'uniswap_v2', 'aerodrome', 'squadswap'] });
    }
  }
  
  const otherTokens = tokenList.filter(t => !majorTokens.includes(t) && !stablecoins.includes(t));
  for (const tName of otherTokens) {
    pairs.push({ t0: TOKENS[tName], t1: TOKENS.WETH, dexes: ['uniswap_v3', 'uniswap_v2', 'aerodrome', 'squadswap'] });
    pairs.push({ t0: TOKENS[tName], t1: TOKENS.USDC, dexes: ['uniswap_v3', 'uniswap_v2', 'aerodrome', 'squadswap'] });
  }
...
class PriceFetcher {
  constructor() {
    ...
    this.squadV3Factory = new ethers.Contract(DEX_ADDRESSES.SQUADSWAP_V3_FACTORY, UNISWAP_V3_FACTORY_ABI, provider);
    this.squadV2Factory = new ethers.Contract(DEX_ADDRESSES.SQUADSWAP_V2_FACTORY, UNISWAP_V2_FACTORY_ABI, provider);
    this.squadV2Router = new ethers.Contract(DEX_ADDRESSES.SQUADSWAP_V2_ROUTER, UNISWAP_V2_ROUTER_ABI, provider);
  }
    ...
    try {
      ...
      if (dexType === 'uniswap_v3') {
        ...
        for (const fee of feeTiers) {
          try {
            poolAddress = await this.squadV3Factory.getPool(token0.address, token1.address, fee);
        ...
        if (poolAddress === ethers.constants.AddressZero) {
          poolAddress = await this.squadV2Factory.getPair(token0.address, token1.address);
        }
      }
  ...
  async getPrice(token0, token1, dexType, tradeSize) {
    ...
    try {
      if (dexType === 'uniswap_v3') {
        ...
        for (const fee of feeTiers) {
          try {
            result = await this.squadV2Router.callStatic.getAmountsOut(amountIn, [token0.address, token1.address]);
            if (result && result[1].gt(0)) return result[1];
          } catch (e) {
            continue;
          }
        }
        if (!result) {
          try {
            result = await this.squadV2Router.getAmountsOut(amountIn, [token0.address, token1.address]);
            return result[1];
          } catch (e) {
            return null;
          }
...
class ArbitrageDetector {
  ...
  async scan() {
    console.log(`\n[${new Date().toISOString()}] Scanning ${VERIFIED_PAIRS.length} pairs with MIN_LIQUIDITY: $${CONFIG.MIN_LIQUIDITY_USD}...`);
