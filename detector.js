...
const AERO_POOLS = [
  ...
  { dex: 'pancakeswap_v3', pairAddress: '0x20CB8f872ae894F7c9e32e621C186e5AFCe82Fd0', token0: TOKENS.AERO, token1: TOKENS.WETH, meta: { feeTiers: [100, 500, 3000, 10000] } } // registered, skipped
];
...
class PriceFetcher {
  constructor() {
    ...
    this.pancakeWarnedThisScan = false;
  }
...
class ArbitrageDetector {
  ...
  async scan() {
    this.prices.resetScanWarnings();

    const allPairs  = [...VERIFIED_PAIRS, ...VIRTUAL_POOLS, ...AERO_POOLS, AERO_USDbC_POOL];
    const allRoutes = [...TRIANGULAR_ROUTES];

    console.log(`\n[${new Date().toISOString()}] Scanning ${allPairs.length} pairs (dynamic + explicit VIRTUAL + explicit AERO + AERO/USDbC) & ${allRoutes.length} triangular routes...`);
...
async function main() {
  ...
  http.createServer((req, res) => {
    ...
    res.end('Arbitrage Bot running: Uniswap V2/V3 + Aerodrome + explicit VIRTUAL/AERO + AERO/USDbC + triangular best-of-venues. Pancake V3 gracefully skipped.\n');
