...
const CONFIG = {
  ...
  POOLS_TO_FETCH_FOR_TOKENS: parseInt(process.env.POOLS_TO_FETCH_FOR_TOKENS) || 500, // How many top pools to scan to find tokens
};
...
// This class first finds all relevant tokens, then finds all pools for those tokens.
class TwoStageDiscovery {
    constructor() {
        this.subgraphs = {
            uniswap_v3: "https://base-subgraph.uniswap.org/v3",
            aerodrome: "https://graph.aerodrome.finance/subgraphs/name/aerodrome-finance/aerodrome-v3",
        };
    }
    ...
    async discoverAllTokens() {
        ...
        for (const [dex, url] of Object.entries(this.subgraphs)) {
            console.log(`[Stage 1] Fetching tokens from ${dex}...`);
            const query = `
            {
            ...
            try {
                ...
                console.error(`[Stage 1 Error] Failed to fetch tokens from ${dex}:`, error.message);
            }
        }
    ...
    async findPoolsForSingleToken(token) {
        ...
        for (const [dex, url] of Object.entries(this.subgraphs)) {
            ...
            try {
                const response = await axios.post(url, { query });
                const foundPools = response.data.data.pools;
                ...
                // It's okay if one query fails, we just log it
                // console.error(`[Stage 2 Error] Failed to find pools for ${token.name} on ${dex}:`, error.message);
            }
        }
        return pools;
    }
...
// ==================== ARBITRAGE DETECTOR ====================
// (This class remains the same, but will now use the TwoStageDiscovery)
class ArbitrageDetector {
    constructor() { 
        this.prices = new PriceFetcher();
        this.discovery = new TwoStageDiscovery();
...
async function main() {
  ...
  http.createServer((req, res) => {
    res.statusCode = 200; res.setHeader('Content-Type', 'text/plain'); res.end('Two-Stage Discovery Arbitrage Bot is running.\n');
