...
class PriceFetcher {
  async getPrice(token0, token1) {
    try {
      // Mock prices for now
      const price1 = ethers.utils.parseUnits('1', token0.decimals);
      const price2 = ethers.utils.parseUnits('1', token1.decimals);
...
class ArbitrageDetector {
  ...
  async scan() {
    ...
    for (const pair of pairs) {
      try {
        ...
        if (!isValid) {
          console.log(`  ✗ ${pair.token0.name}/${pair.token1.name}: Validation failed`);
          continue;
        }

        console.log(`  ✓ ${pair.token0.name}/${pair.token1.name}: Valid tokens`);

        const priceResult = await this.prices.getPrice(pair.token0, pair.token1);
  ...
  calculateDifference(price1, price2, decimals0, decimals1) {
    ...
    const p2 = parseFloat(ethers.utils.formatUnits(price2, decimals1));
    return ((p2 - p1) / p1) * 100;
  }
  ...
  async alert(opportunity) {
    if (CONFIG.WEBHOOK_URL) {
      try {
        const diffStr = typeof opportunity.priceDiff === 'number'
          ? opportunity.priceDiff.toFixed(2)
          : String(opportunity.priceDiff);
        await axios.post(CONFIG.WEBHOOK_URL, {
          content: `🚨 **Arbitrage Opportunity!**\n**${opportunity.pair}** | Profit: ${diffStr}%\n${opportunity.dex0} → ${opportunity.dex1}`,
        });
      } catch (err) {
...
class ArbitrageBot {
  ...
  async start() {
    ...
    try {
      ...
      console.error(`✗ RPC Connection Failed: ${err?.message || String(err)}`);
      process.exit(1);
    }
