...
const CONFIG = {
  ...
  MIN_REALIZABLE_PROFIT_PCT: 0.10,
  SLIPIPAGE_TOLERANCE_BPS: 50,    
  ...
  async getQuote(t0, t1, dex, amount) {
    ...
    try {
      ...
      if (dex === 'v2') {
          ...
          return res[res.length - 1]; // FIX: Pick amountOut from array
      }
      if (dex === 'aero') {
          ...
          for (const d2 of dexes) {
            ...
            if (diff >= CONFIG.PRICE_DIFF_THRESHOLD) {
              ...
              if (realizableProfitPct >= CONFIG.MIN_REALIZABLE_PROFIT_PCT) {
                const msg = `💰 EXECUTE: ${tKeys[i]}/${tKeys[j]} | Realizable: ${realizableProfitPct.toFixed(2)}% | Buy on ${d1} -> Sell on ${d2}`;
                console.log(msg);
