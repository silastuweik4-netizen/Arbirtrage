...
class ArbitrageDetector {
  ...
  async scan() {
    ...
    console.log(`\n[${new Date().toISOString()}] Scanning ${allPairs.length} pairs (dynamic + explicit VIRTUAL) & ${TRIANGULAR_ROUTES.length} triangular routes...`);
...
async function main() {
  ...
  http.createServer((req, res) => {
    ...
    res.end('Arbitrage Bot running: multi-fee Uniswap V3 + explicit VIRTUAL pools + triangular best-of-venues + double-check logic.\n');
