...
async function handleLogMessage(logResult) {
  ...
  if (logs) {
    ...
    if (allLogs.includes('Instruction: LiquidateObligation')) {
      ...
      const borrowerMatch = allLogs.match(/Borrower: (\w+)/i);
      const tokenMatch = allLogs.match(/Token: (\w+)/i);
      const amountMatch = allLogs.match(/Amount: ([\d.]+)/i);
...
app.listen(PORT, () => {
  ...
  // Send a test alert when the server starts
  sendTelegramAlert('✅ Solend Liquidation Tracker started and Telegram alerts are active!');
  connectToSolana();
});
