//  src/memescanner.js  – DexScreener cross-DEX spread hunter
...
const MIN_SPREAD    = Number(process.env.MIN_SPREAD || 1);         // %
const MIN_POOL_LIQ  = Number(process.env.MIN_POOL_LIQ || 3_000);   // USD
...
async function fetchDexPairs(symbol) {
  try {
    ...
    if (!res.ok) { console.log('DexScreener', res.status); return null; }
...
function chooseHighLow(pairs) {
  ...
  return { low: usable[0], high: usable[usable.length - 1], spreadPct: ((usable[usable.length - 1].price - usable[0].price) / usable[0].price) * 100 };
}

async function scanRound() {
  for (const token of TOKENS) {
    const data = await fetchDexPairs(token.trim());
    const bw   = chooseHighLow(data?.pairs);
    if (!bw) continue;

    const lowLiq  = Number(bw.low.liquidity?.usd || 0);
    const highLiq = Number(bw.high.liquidity?.usd || 0);
    console.log(`[${token}] spread=${bw.spreadPct.toFixed(2)}% low=${bw.low.price}(${bw.low.dexId}) liq=$${lowLiq} high=${bw.high.price}(${bw.high.dexId}) liq=$${highLiq}`);

    if (bw.spreadPct >= MIN_SPREAD && lowLiq >= MIN_POOL_LIQ && highLiq >= MIN_POOL_LIQ) {
      const msg = [
        '🚨 Live memecoin arb ≥ ' + MIN_SPREAD + '%',
        `${token.toUpperCase()} — Spread ${bw.spreadPct.toFixed(2)}%`,
        `Buy  @ ${bw.low.price}  (${bw.low.dexId})  — Liq $${lowLiq.toFixed(0)}`,
        `Sell @ ${bw.high.price} (${bw.high.dexId}) — Liq $${highLiq.toFixed(0)}`
      ].join('\n');
      await notify(msg);
    }
    await sleep(600); // gentle rate-limit
  }
}
