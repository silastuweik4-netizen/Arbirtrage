//  arbEngine.js  – Telegram alerts + base-58
...
async function jupQuote(inputMint, outputMint, amount) {
  ...
  const res = await fetch(url); return res.ok ? res.json() : null;
}
...
async function submitJito(txSerialize) {
  ...
  if (!res.ok) throw new Error('Jito error'); const { result } = await res.json(); return result;
}
