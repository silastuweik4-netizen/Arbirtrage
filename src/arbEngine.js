...
const BASE_MINTS = [
  ...
  'Dfh5DzRgSvvCFDoYc2ciTkMrbDfRKybA4SoFbPmApump'  // Pippin (manual find)
];
const LOAN_AMOUNTS = [1_000, 5_000, 20_000].map(v => v * 1e6);
const MIN_SPREAD   = 0.001;                   // 0.1 %
const MIN_ABS_USD  = 25;                      // $25 absolute profit floor
const SCAN_MS      = 500;                     // 500 ms
...
async function microSlotSnipe() {
  for (const baseMint of BASE_MINTS) {
    for (const loan of LOAN_AMOUNTS) {
      ...
      return { loan, spread, profit, safe, q1, q2, baseMint };   // single shot
    }
  }
  return null;
}
...
setInterval(async () => {
  ...
  if (seed) await scanAndArb(seed);   // reuse your existing engine
}, SCAN_MS);

/* ---------- existing engine (unchanged safety chain) ---------- */
