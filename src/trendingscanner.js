//  src/trendingscanner.js  – auto-append within 60 s, no human delay
import fetch from 'node-fetch';
import { notify } from './telegram.js';
import { config } from 'dotenv'; config();

const MIN_SPREAD    = Number(process.env.TREND_MIN_SPREAD || 0.2);
const INTERVAL_MS   = Number(process.env.TREND_INTERVAL || 60000); // 1 min
const lastAlerts    = new Map();
const fmt = n => Number(n).toLocaleString(undefined, { maximumFractionDigits: 6 });

// ===== auto-append inside scrape =====
async function scrapeTrending() {
  ...
  for (const m of matches) {
    ...
    if (spread >= MIN_SPREAD) {
      ...
      // ===== auto-append within same tick =====
      if (!current.includes(mint)) {
        process.env.SCAN_MINTS = [...current, mint].join(',');
...
export async function startTrendingScanner() {
  ...
  async function loop() {
    ...
    for (const r of list) {
      const now = Date.now();
      if (lastAlerts.get(r.mint) && now - lastAlerts.get(r.mint) < 60000) continue;
      const msg =
