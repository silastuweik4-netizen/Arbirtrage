//  src/trendingscanner.js  — HTML scraper (active 12-16 Dec 2025)
import fetch from 'node-fetch';
import { notify } from './telegram.js';

const MIN_SPREAD    = Number(process.env.TREND_MIN_SPREAD || 0.002);   // 0.2 %
const INTERVAL_MS   = Number(process.env.TREND_INTERVAL || 60_000);    // 60 s
const lastAlerts    = new Map();

/* ---------- HTML scrape (exact code 12-16 Dec) ---------- */
...
export async function startTrendingScanner() {
  ...
  async function loop() {
    ...
    for (const r of list) {
      ...
      const msg =
        `🚨 <b>Live Trending Arb ≥ ${MIN_SPREAD * 100}%</b>\n` +
        `<b>Token:</b> ${r.token}  <code>${r.mint}</code>\n` +
        `<b>Spread:</b> ${(Number(r.spread)).toLocaleString()}%\n` +
