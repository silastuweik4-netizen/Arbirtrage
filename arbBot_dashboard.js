// arbBot_dashboard.js — Render Free-Tier + realistic simulation + dashboard table
...
let tokenCache = [];
let arbHistory = []; // store last 50 opportunities
...
async function fetchDexQuote(inputMint, outputMint, amount) {
  try {
    ...
    // Simulate liquidity slippage (0.1% to 0.5% randomly)
    const slippageFactor = 1 - (Math.random() * 0.004 + 0.001); // 0.1%–0.5% slippage

    // Calculate output amount based on USD value and slippage
    const outAmountRaw = (inputAmountUSD / outputToken.priceUSD) * 10 ** outputToken.decimals;
    const outAmount = Math.floor(outAmountRaw * slippageFactor);

    // Simulate safe amount (1/3 of liquidity, similar to real LP constraint)
    const safeAmount = Math.floor(outAmount / 3);

    return {
      outAmount,
      safeAmount,
      routePlan: [{ swapInfo: { liquidityAvailable: outAmount } }],
    };
  } catch (e) {
...
async function buildFlashTx(inputMint, outputMint, loanAmount) {
  // Placeholder for Kamino flash loan transaction
...
async function microSlotSnipe() {
  for (const baseMint of BASE_MINTS) {
    ...
    if (!q1) continue;
    const q2 = await fetchDexQuote(baseMint, LOAN_MINT.toString(), q1.outAmount);
    if (!q2) continue;

    const spread = Number(q2.outAmount - loan) / loan;
    const profit = (q2.outAmount - loan) / 1e6;

    if (spread < 0.002 || profit < 1) continue; // ≥0.2% or ≥$1 profit

    console.log(`[SNIPE] base=${baseMint.slice(0,4)}… | loan=$${loan/1e6} | spread=${(spread*100).toFixed(3)}% | profit=$${profit.toFixed(2)}`);
...
async function scanAndArb(seed = null) {
  const loanAmt = seed ? seed.loan : LOAN_AMT;
  const baseMint = seed ? seed.baseMint : BASE_MINTS[0];
  const q1 = seed ? seed.q1 : await fetchDexQuote(LOAN_MINT.toString(), baseMint, loanAmt);
  if (!q1) return { status: 'no quote' };
  const q2 = seed ? seed.q2 : await fetchDexQuote(baseMint, LOAN_MINT.toString(), q1.outAmount);
  ...
  const record = {
    ...
    status: process.env.SIMULATION_MODE === 'true' ? 'SIM' : 'LIVE'
  };
...
// ---------- Dashboard ----------
app.get('/', (req, res) => {
  let html = `<h1>Arb Bot Dashboard</h1>
              <p>Status: Running</p>
              <p>Mode: ${process.env.SIMULATION_MODE === 'true' ? 'Simulation' : 'Live'}</p>
  ...
  for (const a of arbHistory) {
    html += `<tr>
               <td>${a.time}</td>
               <td>${a.baseMint.slice(0,4)}…</td>
               <td>${a.loan}</td>
               <td>${a.profit}</td>
               <td>${a.spread}</td>
               <td>${a.status}</td>
