...
const JUP_API = 'https://lite-api.jup.ag/swap/v1/quote';
const RAYDIUM_API = 'https://api.raydium.io/v2/sdk/liquidity';
const ORCA_API = 'https://api.orca.so/pool';
...
/* ===================== CACHE HELPERS ===================== */
function getCacheKey(dex,inputMint,outputMint,amount){return `${dex}-${inputMint}-${outputMint}-${amount}`;}
function setCache(key,data){quoteCache.set(key,{data,timestamp:Date.now()});}
function getCache(key){const cached=quoteCache.get(key);if(!cached)return null;if(Date.now()-cached.timestamp>CACHE_TTL){quoteCache.delete(key);return null;}return cached.data;}

/* ===================== FETCH HELPERS ===================== */
async function retryFetch(fn,retries=MAX_RETRIES){let lastErr;for(let i=0;i<=retries;i++){try{return await fn();}catch(e){lastErr=e;await new Promise(r=>setTimeout(r,50));} }throw lastErr;}
async function getUsdcBalance(){const ata=await conn.getTokenAccountsByOwner(keypair.publicKey,{mint:LOAN_MINT});if(!ata.value.length)return 0;const bal=await conn.getTokenAccountBalance(ata.value[0].pubkey);return Number(bal.value.amount);}
...
async function fetchDexQuote(dex,inputMint,outputMint,amount){
  ...
  const data=await retryFetch(async()=>{
    let url;
    if(dex==='JUP') url=`${JUP_API}?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amount}&slippageBps=50`;
    if(dex==='RAY') url=`${RAYDIUM_API}?from=${inputMint}&to=${outputMint}&amount=${amount}`;
    if(dex==='ORCA') url=`${ORCA_API}?from=${inputMint}&to=${outputMint}&amount=${amount}`;
    const res=await fetch(url,{signal:AbortSignal.timeout(2000)});
    if(!res.ok)throw new Error(`${dex} fetch failed`);
    const q=await res.json();
    if(!q.routePlan && dex==='JUP') throw new Error('No routePlan');
    q.dex=dex;
    q.safeAmount=Math.floor(Math.min(...(q.routePlan?.map(h=>h.swapInfo.liquidityAvailable)||[amount]))/3);
    return q;
  });
  setCache(key,data);
  return data;
...
async function buildFlashTx(inputMint,outputMint,loanAmount){
  const body={token:inputMint.toString(),amount:loanAmount,user:keypair.publicKey.toString(),instructions:'jupiter',inputMint:inputMint.toString(),outputMint:outputMint.toString(),finalMint:inputMint.toString(),slippageBps:50,priorityFeeLamports:Math.max(10000,Math.floor(loanAmount*0.0001))};
  const res=await fetch('https://api.kamino.finance/v1/flash-loan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  if(!res.ok)return null;const {tx}=await res.json();return Buffer.from(tx,'base64');}

async function submitJito(txSerialize){
  const payload={jsonrpc:'2.0',id:1,method:'sendBundle',params:[[Array.from(txSerialize)]]};
  const res=await fetch('https://mainnet.block-engine.jito.wtf/api/v1/bundles',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${JITO_AUTH}`},body: JSON.stringify(payload)});
  if(!res.ok)throw new Error('Jito bundle error');const {result}=await res.json();return result;
}
...
app.use(express.static('public')); // public/index.html dashboard
