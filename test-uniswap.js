 ...
 * Uses staticCallResult so read-only provider works
 */
...
(async () => {
  ...
  // USDC → USDbC (0.01 % pool – exists on Base) ------------------------------------
  ...
  // ABI for the separate-argument overload on Base -------------------------------
  const QUOTER_ABI = [
  ...
  try {
    // MUST use staticCallResult – provider is read-only --------------------------
    const amountOut = await quoter.quoteExactInputSingle.staticCallResult(tokenIn, tokenOut, fee, amountIn, 0);
