...
const MIN_LIQUIDITY_USD = 50000; // Only scan pairs with more than $50k liquidity to avoid noise
...
// The main function that scans the market
async function scanMarketForArbitrage() {
  console.log(`--- [${new Date().toISOString()}] Starting full market scan ---`);

  try {
    // 1. Fetch the top pairs on Solana from Dexscreener
    const response = await axios.get('https://api.dexscreener.com/latest/dex/chains/solana');
    ...
    console.log(`Found ${tokenGroups.size} tokens to analyze after filtering.`);
    ...
    for (const [tokenAddress, tokenPairs] of tokenGroups) {
      ...
      if (minPrice !== Infinity && maxPrice > 0 && buyDex !== sellDex) {
        ...
        if (percentDifference > ARBITRAGE_THRESHOLD_PERCENT) {
          ...
          // await executeKaminoFlashloanAndMEV(tokenAddress, buyDex, sellDex, ...);
    ...
    console.error("Error during market scan:", error.message);
  }
}

// --- Initialization ---

console.log("Starting advanced market scanner...");

// *** NOTE: This is more intensive. Consider running every 10 minutes. ***
// 10 minutes = 600,000 milliseconds
setInterval(scanMarketForArbitrage, 600000);

// Run it once immediately on startup
scanMarketForArbitrage();
