...
const QUOTERS = {
    // Official Aerodrome Slipstream Quoter
    AERODROME: "0x254cF9E1E6e233aa1AC962CB9B05b2cfeAaE15b0",
    // Official PancakeSwap V3 QuoterV2
    PANCAKESWAP: "0xB048Bbc1Ee6b733FFfCFb9e9CeF7375518e25997"
};

const QUOTER_ABI = [
    // PancakeSwap V3 QuoterV2 signature
    "function quoteExactInputSingle(tuple(address tokenIn, address tokenOut, uint256 amountIn, uint24 fee, uint160 sqrtPriceLimitX96)) external returns (uint256 amountOut, uint160 sqrtPriceX96After, uint32 initializedTicksCrossed, uint256 gasEstimate)",
    // Aerodrome Slipstream path-based signature
    "function quoteExactInput(bytes path, uint256 amountIn) external returns (uint256 amountOut, uint160[] sqrtPriceX96AfterList, uint32[] initializedTicksCrossedList, uint256 gasEstimate)"
];

// CORRECTED: Manually assigned array values to stop SyntaxError
...
class ArbitrageScanner {
    ...
    async getQuote(dex, tokenIn, tokenOut, fee, amount) {
        try {
            if (dex === 'PANCAKE') {
                // PancakeSwap requires a struct (tuple) input
