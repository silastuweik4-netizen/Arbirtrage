...
// --- CONFIGURATION ---
// Using static network config to bypass network detection errors
...
const QUOTERS = {
    AERODROME: "0x254cF9E1E6e233aa1AC962CB9B05b2cfeAaE15b0", // Aerodrome Slipstream Quoter
    PANCAKESWAP: "0xB048Bbc1Ee6b733FFfCFb9e9CeF7375518e25997" // PancakeSwap V3 Quoter
};
...
// 2026 standard fee tiers: 100=0.01%, 500=0.05%, 2500=0.25%, 3000=0.3%, 10000=1%
const FEE_TIERS = ;
...
class ArbitrageScanner {
    ...
    async initialize() {
        try {
            ...
            console.log(`✅ [CORE] Scanner Linked to Base. Block: ${block}`);
        } catch (e) {
            console.error("❌ [ERROR] Connection failed. Check RPC URL.");
        }
    }
    ...
    async getQuote(contract, tokenIn, tokenOut, fee, amount) {
        try {
            ...
            return 0n; // Revert means no pool exists for this fee tier
        }
    }

    async scanForArbitrageOpportunities() {
        const opportunities = [];
        const amountIn = ethers.parseEther("1"); // Testing with 1 unit
        ...
        console.log(`\n🔍 [SCAN START] - Checking ${pairsToScan.length} pairs at ${new Date().toLocaleTimeString()}`);
            ...
            if (bestAero > 0n || bestPancake > 0n) {
                ...
                if (bestAero > 0n && bestPancake > 0n) {
                    ...
                    if (percentage >= 0.01) { // 0.01% threshold
                        ...
                        console.log(`   ✨ [OPPORTUNITY] ${pair.name}: ${percentage.toFixed(4)}% spread detected!`);
                    }
                }
            }
        }
