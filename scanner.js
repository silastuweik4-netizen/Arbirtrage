...
const UNISWAP_FACTORY = '0x33128a8fC17869897dcE68Ed026d694621f6FDaD';
...
class ArbitrageScanner {
  ...
  async initialize() {
    try {
      ...
      console.log(`🏢 DEXes: Aerodrome, PancakeSwap, Uniswap V3`);
  ...
  logPairPrices(pairLabel, aeroPrice, panPrice, uniPrice, direction) {
    ...
    const uniStr = uniPrice ? `Uni: ${uniPrice.price.toFixed(6)}` : 'Uni: ✗';
    
    this.log(`    📊 ${pairLabel} (${direction})`);
    this.log(`       ${aeroStr} | ${panStr} | ${uniStr}`);
  }

  logPairAvailability(pairLabel, aeroPrice, panPrice, uniPrice, direction) {
    const available = [];
    if (aeroPrice) available.push('Aero');
    if (panPrice) available.push('PanCake');
    if (uniPrice) available.push('Uni');
    
    if (available.length > 0) {
      this.log(`    📍 ${pairLabel} (${direction}): Found on ${available.join(', ')}`);
    } else {
      this.log(`    ❌ ${pairLabel}: No pairs found`);
    }
  }
  ...
  async findMatchingPairs(token0, token1) {
    // Try to find the same pair on all three DEXes
    ...
    const uniPrice = await this.getPairPrice('Uniswap', this.uniswapFactory, token0, token1);

    // If all don't exist, try reverse
    let aeroReversePrice = null;
    let panReversePrice = null;
    let uniReversePrice = null;
    ...
    if (!uniPrice) {
      uniReversePrice = await this.getPairPrice('Uniswap', this.uniswapFactory, token1, token0);
    }

    return {
      forward: { aero: aeroPrice, pan: panPrice, uni: uniPrice },
      reverse: { aero: aeroReversePrice, pan: panReversePrice, uni: uniReversePrice }
    };
  }
  ...
  async scanForArbitrageOpportunities() {
    ...
    try {
      ...
      for (const [tokenA, tokenB] of tokenPairs) {
        ...
        this.log(`\n  🔎 Checking: ${pairLabel} (${this.requestCount} requests used)`);

        const pairs = await this.findMatchingPairs(tokenA.address, tokenB.address);

        // Check forward direction
        if (pairs.forward.aero && pairs.forward.pan) {
          this.logPairPrices(pairLabel, pairs.forward.aero, pairs.forward.pan, pairs.forward.uni, 'forward');
          ...
          if (opp) {
            this.log(`    ✅ Opportunity: ${opp.priceDiffPercent}% | ${opp.cheaperOn} → ${opp.expensiveOn} | Profit: ${opp.profitPotential}%`);
            opportunities.push(opp);
          }
        } else {
          this.logPairAvailability(pairLabel, pairs.forward.aero, pairs.forward.pan, pairs.forward.uni, 'forward');
        }

        // Check reverse direction
        if (pairs.reverse.aero && pairs.reverse.pan) {
          this.logPairPrices(pairLabel, pairs.reverse.aero, pairs.reverse.pan, pairs.reverse.uni, 'reverse');
          const opp = this.analyzeOpportunity(
            tokenA,
            tokenB,
            pairs.reverse.aero,
          ...
          if (opp) {
            this.log(`    ✅ Opportunity (reverse): ${opp.priceDiffPercent}% | ${opp.cheaperOn} → ${opp.expensiveOn} | Profit: ${opp.profitPotential}%`);
            opportunities.push(opp);
          }
        }
      }
  ...
  analyzeOpportunity(tokenA, tokenB, aeroPrice, panPrice, uniPrice, direction) {
    // Find the pair with best prices
    const prices = [
      { dex: 'Aerodrome', price: aeroPrice.price, data: aeroPrice },
      { dex: 'PancakeSwap', price: panPrice.price, data: panPrice }
    ];

    if (uniPrice) {
      prices.push({ dex: 'Uniswap', price: uniPrice.price, data: uniPrice });
    }
    ...
    const percentDiff = ((maxPrice.price - minPrice.price) / minPrice.price) * 100;
    ...
    return {
      ...
      uniswapPrice: uniPrice ? uniPrice.price.toFixed(8) : 'N/A',
      priceDiffPercent: percentDiff.toFixed(3),
      cheaperOn: minPrice.dex,
      expensiveOn: maxPrice.dex,
      aerodromeReserves: aeroPrice.reserve0 ? { reserve0: aeroPrice.reserve0, reserve1: aeroPrice.reserve1 } : null,
      pancakeswapReserves: panPrice.reserve0 ? { reserve0: panPrice.reserve0, reserve1: panPrice.reserve1 } : null,
      uniswapReserves: uniPrice && uniPrice.reserve0 ? { reserve0: uniPrice.reserve0, reserve1: uniPrice.reserve1 } : null,
