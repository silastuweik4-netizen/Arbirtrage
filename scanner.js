...
async function scanMarketForArbitrage() {
  ...
  try {
    // 1. Fetch the TOP trending pairs from Dexscreener (Corrected URL)
    const response = await axios.get('https://api.dexscreener.com/latest/dex/trending');
    const pairs = response.data.pairs;

    // 2. Filter for quality pairs and group them by the base token
    const tokenGroups = new Map();
    for (const pair of pairs) {
      // *** NEW FILTER: Only look at pairs on the Solana chain ***
      ...
      if (isOnSolana && isStablecoinQuoted && hasLiquidity) {
    ...
    console.log(`Found ${tokenGroups.size} Solana tokens to analyze after filtering.`);
...
setInterval(scanMarketForArbitrage, 600000); // Run every 10 minutes
scanMarketForArbitrage(); // Run once on startup
