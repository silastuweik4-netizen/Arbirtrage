...
async function sendTelegramAlert(message) {
  ...
  try { await axios.post(telegramUrl, { chat_id: CHAT_ID, text: message, parse_mode: 'Markdown' }); console.log("✅ Alert sent to Telegram."); } 
  catch (error) { console.error("Failed to send Telegram alert:", error.response?.data || error.message); }
}

// --- SUPER HYBRID DETECTION LOGIC ---
...
async function superHybridScan() {
  ...
  for (const item of allSearches) {
    try {
      const query = item.mintAddress || item.symbol;
      console.log(`\nSearching for: ${query}`);
  ...
  for (const [tokenAddress, tokenPairs] of tokenGroups) { if (tokenPairs.length < 2) continue; let minPrice = Infinity; let maxPrice = 0; let buyDex = null; let sellDex = null; for (const pair of tokenPairs) { const price = parseFloat(pair.priceUsd); if (price < minPrice) { minPrice = price; buyDex = pair.dexId; } if (price > maxPrice) { maxPrice = price; sellDex = pair.dexId; } } if (minPrice !== Infinity && maxPrice > 0 && buyDex !== sellDex) { const percentDifference = ((maxPrice - minPrice) / minPrice) * 100; if (percentDifference > ARBITRAGE_THRESHOLD_PERCENT) { const tokenSymbol = tokenPairs[0].baseToken.symbol; console.log(`🚨 Opportunity found for ${tokenSymbol}: ${percentDifference.toFixed(2)}%`); await sendTelegramAlert(`Opportunity for ${tokenSymbol}: ${percentDifference.toFixed(2)}%`); } } }
...
setInterval(superHybridScan, 600000); // Run every 10 minutes due to more API calls
superHybridScan();
