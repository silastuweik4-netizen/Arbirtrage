...
// FIXED: Defined standard fee tiers (100 = 0.01%, 500 = 0.05%, 2500/3000 = 0.25/0.3%, 10000 = 1%)
    ...
    async getQuote(dex, tokenIn, tokenOut, fee, amountInScaled) {
        try {
            if (dex === 'PANCAKE') {
                ...
                // Using staticCall to get return values from a state-changing function without a transaction
                return (await this.pancake.quoteExactInputSingle.staticCall(params)).amountOut;
            } else {
                // Aerodrome/Uniswap Path encoding: [address, fee, address]
    ...
    async scanForArbitrageOpportunities() {
        ...
        for (const pair of pairsToScan) {
            ...
            if (bestAero > 0n && bestPancake > 0n) {
                ...
                // Log if spread is significant (at least 0.01%)
                if (spread >= 0.01) {
                    opportunities.push({ pair: pair.name, spreadPercent: spread.toFixed(4) });
                }
            }
        }
