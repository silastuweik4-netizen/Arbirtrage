...
class ArbitrageScanner {
  ...
  async getTokenInfo(addr) {
    try {
      const checksumAddr = ethers.getAddress(addr); // Normalize to checksum
  ...
  async _getV3QuotedPrice(factory, quoter, tokenIn, tokenOut, dexName) {
    ...
    const key = `${dexName}-${checksumIn.toLowerCase()}-${checksumOut.toLowerCase()}`;
    if (this.pairCache.has(key)) return this.pairCache.get(key);

    const fees = [100, 500, 2500, 10000];
    const amountIn = ethers.parseUnits('1', (await this.getTokenInfo(checksumIn)).decimals); // 1 unit input

    for (const fee of fees) {
      try {
        const poolAddr = await factory.getPool(checksumIn, checksumOut, fee);
        if (poolAddr === ethers.ZeroAddress) continue;

        // Use Quoter to get exact output
        const amountOut = await quoter.quoteExactInputSingle(checksumIn, checksumOut, fee, amountIn, 0n);

        if (amountOut === 0n) continue;

        const decIn = (await this.getTokenInfo(checksumIn)).decimals;
        const decOut = (await this.getTokenInfo(checksumOut)).decimals;
  ...
  async getAerodromePrice(WETH, tokenAddr) {
    const checksumWETH = ethers.getAddress(WETH);
    const checksumToken = ethers.getAddress(tokenAddr);
    // Try Slipstream first (main liquidity) with Quoter, fallback to V2 reserves
    let price = await this._getV3QuotedPrice(this.aeroSlipstreamFactory, this.aeroSlipstreamQuoter, checksumWETH, checksumToken, 'Aerodrome-Slipstream');
    if (!price) {
      price = await this._getV2Price(this.aeroV2Factory, checksumWETH, checksumToken, 'Aerodrome-V2');
    }
    return price;
  }
  ...
  async getPancakePrice(WETH, tokenAddr) {
    const checksumWETH = ethers.getAddress(WETH);
    const checksumToken = ethers.getAddress(tokenAddr);
    return this._getV3QuotedPrice(this.pancakeFactory, this.pancakeQuoter, checksumWETH, checksumToken, 'PancakeSwap');
  }
